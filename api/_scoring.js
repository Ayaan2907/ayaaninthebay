...
//      the same EventScorer shape later takes a jev scorer (needs TYPESAFE_API_KEY) or the
//      structured-output llm (llmScore below, when the ai provider is on).
...
//      stalls on the llm.
// pure module: no env, no fetch, no log. the handler in api/score.js wires the world in.
...
const eventText = (e) => textOf([e && e.title, e && e.note, e && e.venue, e && e.category, e && e.address]);
const profileText = (p) =>
...
// then, and the handler says so instead of pretending).
function retrieve(profileStr, records, k = records.length) {
  const hasWords = tokenize(profileStr).length > 0;
  const q = hasWords ? embed(profileStr) : null;
  return records
    .map((record) => ({ record, fit: q ? Math.max(0, round2(cosine(q, embed(eventText(record))))) : 0 }))
...
const cleanList = (v, maxItems = 12, maxLen = 80) =>
  Array.isArray(v)
    ? [...new Set(v.map((x) => oneLine(x, maxLen)).filter(Boolean))].slice(0, maxItems)
    : [];

// structured profile pasted or remembered client-side. allow-list only.
function parseProfileInput(v) {
  if (!v || typeof v !== "object") return null;
  const name = oneLine(v.name, 120);
  const headline = oneLine(v.headline, 200);
  const roles = cleanList(v.roles);
  const topics = cleanList(v.topics);
  const goals = cleanList(v.goals);
  const links = {};
  for (const [k, u] of Object.entries(v.links && typeof v.links === "object" ? v.links : {})) {
    if (["linkedin", "github", "x", "site"].includes(k) && /^https:\/\//.test(String(u))) links[k] = String(u).slice(0, 300);
  }
  if (!name && !headline && !roles.length && !topics.length && !goals.length && !Object.keys(links).length) return null;
...
function parseSourceInput(v) {
  if (!v || typeof v !== "object") return null;
  if (v.kind === "linkedin_url") {
    const url = String(v.value || "").trim();
    if (!/^https:\/\/(www\.)?linkedin\.com\/(in|pub|company)\/[^\s?#]+/i.test(url) || url.length > 300) return null;
...
function heuristicScore({ profile, event, goal, meets = [], now = Date.now(), fit } = {}) {
  const p = profile || {};
  const eText = eventText(event);
  const pText = profileText(p);
  const pFit = fit != null ? fit : tokenize(pText).length ? Math.max(0, cosine(embed(pText), embed(eText))) : 0;
  const gFit = goal && tokenize(goal).length ? Math.max(0, round2(cosine(embed(goal), embed(eText)))) : 0;
  ...
  const go = round2(clamp(0.32 + 0.40 * pFit + 0.18 * gFit + 0.10 * net + 0.04 * soon, 0.02, 0.97));
...
const verdictOf = (go) => (go >= 0.6 ? "go" : go >= 0.4 ? "maybe" : "skip");
...
const segmentFor = (event) => SEGMENTS[event.category] || SEGMENTS.events;

// the profile words that appear on the event, for a reason you can check by eye.
function sharedTokens(profile, event, goal, max = 3) {
...
function fallbackExplain(heuristic, { profile, event, goal, meets = [] } = {}) {
  const p = profile || {};
  ...
  else if (heuristic.go >= 0.4) outcome = `coin toss: the room is ${seg}, and whether that helps depends on what you want that night`;
  else outcome = `likely a skip: the crowd (${seg}) does not connect to much on your profile`;

  const meet = meets.length
    ? meets.slice(0, 3)
    : [{ who: seg, why: "this category of event draws them", starter: "ask what brought them out and what they are working on" }];
...
const strArr = (v, max, maxLen) =>
  Array.isArray(v) ? v.filter((x) => typeof x === "string" && x.trim()).map((x) => x.trim().slice(0, maxLen)).slice(0, max) : [];
...
async function llmScore({ chat, model, heuristic, profile, event, goal, meets = [], fitRank } = {}) {
  if (typeof chat !== "function") throw new Error("no chat function");
  const user = JSON.stringify({
    profile: { headline: profile && profile.headline, roles: profile && profile.roles, topics: profile && profile.topics, goals: profile && profile.goals, raw: profile && profile.raw },
    goal: goal || "(none given)",
    event: { title: event.title, category: event.category, venue: event.venue, note: event.note, startsAt: event.startsAt, url: event.url },
  ...
  const meet = (Array.isArray(j.meet) ? j.meet : [])
    .map((m) => (m && typeof m === "object" ? { who: oneLine(m.who, 120), why: oneLine(m.why, 200), starter: oneLine(m.starter, 200) } : null))
    .filter((m) => m && m.who)
    .slice(0, 3);
